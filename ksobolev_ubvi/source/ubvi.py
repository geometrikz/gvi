...
class UBVI(BVI):
    def __init__(
        ...
        self.n_logfg_samples = (
            n_logfg_samples  # number of samples for posterior estimation
        )
    ...
    def _compute_weights(self):
        ...
        if self.params.shape[0] == 1:
            ...
            b = torch.Tensor(
                nnls(
                    Linv.detach().numpy(),
                    -torch.einsum("ij,j->i", Linv, d).detach().numpy(),
                )[0]
            )
            lbd = torch.einsum("ij,j->i", Linv, b + d)
            w = torch.max(
                torch.zeros(1),
                torch.einsum("ij,j->i", Linv.T, lbd / torch.sqrt(((lbd**2).sum()))),
            )

        # compute weighted logfg sum
        self._logfgsum = torch.logsumexp(
            torch.cat(
                (
    ...
    def _objective(self, x, itr):
        ...
        if self.weights.shape[0] == 0:
            ...
            lgh = torch.logsumexp(
                ...
                + atleast_2d(
                    self.component_dist.log_sqrt_pair_integral(x, self.params)
                )[:, self.weights > 0],
                (0, 1),
            )

        h_samples = self.component_dist.generate_samples_for_one_component(
            x, self.n_samples
        )
        ...
        log_denom = 0.5 * torch.log(1.0 - torch.exp(torch.tensor(2 * lgh)))
        if lf_num > lg_num:
            logobj = lf_num - log_denom + torch.log(1.0 - torch.exp(lg_num - lf_num))
            neglogobj = -logobj
            return neglogobj
        else:
            if not allow_negative:
                return float("inf")
            lognegobj = lg_num - log_denom + torch.log(1.0 - torch.exp(lf_num - lg_num))
            return lognegobj

    def _logfg_est(self, param):
        samples = self.component_dist.generate_samples_for_one_component(
            param, self.n_samples
        )
        lf = 0.5 * self.target_log_pdf(samples)
    ...
    def _logg(self, samples):
        ...
        return torch.logsumexp(
            logg_x[:, self.weights > 0] + torch.log(self.weights[self.weights > 0]),
            dim=1,
        )
    ...
    def _sample_g(self, n):
        ...
        for j in range(self.Z.shape[0]):
            for m in range(j + 1):
                ...
                g_samples[
                    cur_idx : cur_idx + n_samps, :
                ] = self.component_dist.generate_samples_for_paired_distribution(
    ...
    def _get_mixture(self):
        ...
        for i in range(self.N):
            for j in range(self.N):
                paired_params[
                    i * self.N + j, :
                ] = self.component_dist._get_paired_param(
                    self.params[i, :], self.params[j, :], flatten=True
                )
