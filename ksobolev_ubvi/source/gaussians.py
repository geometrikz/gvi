...
class Gaussian(object):
    ...
    def log_pdf(self, params, X):
        ...
        log_pdf = (
            -0.5 * mus.shape[1] * torch.log(torch.tensor(2 * math.pi))
    ...
    def log_sqrt_pair_integral(self, new_param, old_params):
        ...
        log_sigmas_all = torch.log(torch.tensor(0.5)) + torch.logsumexp(
        ...
        return (
            -0.125
            * torch.sum(torch.exp(-log_sigmas_all) * (mu_new - mus_old) ** 2, axis=1)
            - 0.5 * torch.sum(log_sigmas_all, axis=1)
            + 0.25 * torch.sum(log_sigma_new)
    ...
    def params_init(self, params, weights, inflation):
        ...
        if i == 0:
            ...
            log_sigmas = params[:, self.d :]
            mu = mus[k, :] + torch.randn(self.d) * torch.sqrt(
                torch.tensor(inflation, dtype=torch.float32)
